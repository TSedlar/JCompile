...
public enum OperatingSystem {
    ...
    public static String getJavaDeveloperKitPath() {
        String bootPath = System.getProperty("sun.boot.class.path");
        String[] splits = bootPath.split(File.pathSeparator);
        for (String split : splits) {
            if (split.contains("jdk")) {
                int jdkIndex = split.indexOf("jdk");
                int separatorIndex = split.indexOf(File.separator, jdkIndex);
                return split.substring(0, separatorIndex + 1);
            }
        }
        return bootPath;
    }
